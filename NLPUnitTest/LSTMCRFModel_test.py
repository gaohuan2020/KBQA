...
class LSTMCRFModel_test(unittest.TestCase):
    ...
    def test_lstmCRFModel(self):
        with torch.no_grad():
            ...
            precheck_tags = torch.tensor(
                [self.tag_to_ix[t] for t in self.training_data[0][1]],
                dtype=torch.long)
            predict = self.model(precheck_sent)
            self.assertEqual(len(predict), 2, 2)
            self.assertEqual(len(predict[1]), len(precheck_tags),
                             len(precheck_tags))
    ...
    def test_lstmCRFModelTrainAndTest(self):
        ...
        for i in trange(100):
            ...
            sentence_in_pad, targets_pad = prepare_sequence_batch(
                self.training_data, self.word_to_ix, self.tag_to_ix)
            loss = self.model.neg_log_likelihood_parallel(
                sentence_in_pad, targets_pad)
            loss.backward()
            if i == 0:
                startLoss = loss.item()
            self.optimizer.step()
    ...
    def test_dataLoaderAndTrainer(self):
        train_dataset = NERDataSet(self.word_to_ix, self.tag_to_ix,
                                   self.training_data)
        training_data_loader = DataLoader(
            train_dataset,
            batch_size=LSTMCRFModelConfig['trainBatchSize'],
            num_workers=LSTMCRFModelConfig['threads'],
            shuffle=LSTMCRFModelConfig['isShuffle'])
        testing_data_loader = DataLoader(
            train_dataset,
            batch_size=LSTMCRFModelConfig['testBatchSize'],
            num_workers=LSTMCRFModelConfig['threads'],
            shuffle=LSTMCRFModelConfig['isShuffle'])
